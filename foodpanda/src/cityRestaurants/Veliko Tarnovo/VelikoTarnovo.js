...
class VelikoTarnovo extends Component {
    state = {
        VelikoTarnovo: [
    ...
    render() {
        return (
            <React.Fragment>
            <HeroSection name="Veliko Tarnovo"/>
            <section class="restaurants">
                <div>
                    <h1 class="h1-restaurants"> Veliko Tarnovo </h1>
                    <h2> Популярни ресторанти </h2>
                </div>
                <div className="RestaurantList">
                    {this.state.VelikoTarnovo.map(restaurant => (
                        <RestaurantCard id={restaurant.id} name={restaurant.name} price={restaurant.price} rating={restaurant.rating} image={restaurant.image} city="Veliko Tarnovo" />
                    ))}
                </div>
            </section>
            <DownloadApp/>
